...
export default function SocialFeedMock() {
  ...
  return (
    <section className="py-20 px-6 relative bg-gradient-to-b from-white via-gray-50 to-gray-100 text-center overflow-hidden">
      {/* Subtle background lighting */}
      <div className="absolute -top-32 left-0 w-[36rem] h-[36rem] bg-gradient-to-br from-[#7f00ff]/10 to-blue-500/10 blur-[120px]" />
      <div className="absolute bottom-0 right-0 w-[36rem] h-[36rem] bg-gradient-to-tr from-pink-500/10 to-[#7f00ff]/10 blur-[120px]" />

      <motion.h2 {...fadeIn()} className="text-4xl font-semibold mb-6">
        Experience <span className="text-[#7f00ff]">Web3 Social Media</span>
      </motion.h2>
      <motion.p
        {...fadeIn(0.2)}
        className="text-gray-600 mb-16 max-w-3xl mx-auto text-lg"
      >
        Discover how decentralized engagement feels, transparent, rewarding,
          ...
          >
            ...
            <div className="relative bg-white/80 backdrop-blur-xl border border-gray-200 rounded-3xl p-6 shadow-md hover:shadow-xl transition-all duration-500">
              ...
              <div className="flex justify-between text-gray-500 text-sm">
                ...
                >
                  <Heart className="w-4 h-4 text-[#7f00ff]" /> {post.likes}
                </motion.div>
                <motion.div
                  whileHover={{ scale: 1.1 }}
                  className="flex items-center gap-2 cursor-pointer"
                >
                  <MessageCircle className="w-4 h-4 text-[#7f00ff]" />{" "}
                  {post.comments}
                </motion.div>
                <motion.div
                  whileHover={{ scale: 1.1 }}
                  className="flex items-center gap-2 cursor-pointer"
                >
                  <Coins className="w-4 h-4 text-[#7f00ff]" /> {post.rewards}
                </motion.div>
              </div>
            </div>
          </motion.div>
